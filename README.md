...
2. **과거 백테스트**: 기간(1년·2년·5년·전체·직접)과 전략을 고르고 실행하면, 계산이 끝나는 즉시 수익 요약·매수/매도 차트·매매 기록이 나타납니다.
   - 전략 11개: 매수 후 보유, 이동평균, AI 방향 예측, 골든크로스, 이동평균 정배열, MACD, 신고가 돌파, RSI 과매도·과매수, 볼린저 밴드, 헤드 앤 숄더(차트에 어깨·머리·넥라인 표시), 펌핑 시그널. 전략마다 설정값을 바꿀 수 있습니다.
   - **모든 전략 비교**: 같은 기간·비용으로 11개 전략을 한 번에 계산해 수익률 순위표로 보여주고, '자세히'로 해당 전략의 상세 결과를 엽니다.
   - 모든 신호는 그날 종가까지의 데이터로 정하고 다음 거래일 시가에 체결합니다(`lab/strategies.py`).
...
4. 차트는 캔들·OHLC 막대·선·영역으로 바꿔 볼 수 있고, 5·20·60·120일 이동평균선을 겹쳐 볼 수 있습니다. 마우스 휠로 확대·축소, 드래그로 이동하며 가격축은 보이는 구간에 맞춰집니다.
5. **실험 기록**에서 실험을 하나씩 또는 여러 개 골라 삭제할 수 있습니다(결과 파일도 함께 삭제).
